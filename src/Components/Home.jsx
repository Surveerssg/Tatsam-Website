...
const Home = ({ mode, setMode }) => {
  ...
  return (
    <div className={`min-h-screen w-full transition-colors duration-300 ${mode === 'dark' ? 'bg-gray-900 text-white' : 'bg-white text-gray-900'}`}>
      ...
      <section className="relative min-h-screen flex items-center justify-center px-4 z-10">
        ...
        <div className="text-center max-w-4xl mx-auto">
          ...
          <div className="mt-10 flex flex-wrap justify-center gap-4">
            <Link
              to="/about-us"
              className={`px-8 py-3 rounded-lg font-medium transition-all transform hover:scale-105 ${mode === 'dark'
            ...
            <Link
              to="/join"
              className={`px-8 py-3 rounded-lg font-medium transition-all transform hover:scale-105 hover:text-white ${mode === 'dark'
      ...
      <section ref={nextSectionRef} className="py-20 relative overflow-hidden">
        ...
        <div className="absolute top-0 left-1/2 transform -translate-x-1/2 w-full h-full pointer-events-none">
          ...
          <div className="absolute top-0 left-1/2 transform -translate-x-1/2 w-96 h-96">
            <div className={`absolute inset-0 rounded-full ${mode === 'dark' ? 'bg-yellow-300' : 'bg-orange-300'
        ...
        mode === 'dark' ? 'to-yellow-400' : 'to-orange-400'
          ...
          <div className="absolute top-40 left-1/2 transform -translate-x-1/2 w-full h-64">
            <div className={`absolute inset-0 rounded-full ${mode === 'dark' ? 'bg-purple-400' : 'bg-pink-400'
      ...
      mode === 'dark' ? 'text-white' : 'text-gray-800'
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            {featuredEvents.map((event, index) => (
              <div
                key={index}
                className={`rounded-xl overflow-hidden transition-all hover:transform hover:scale-[1.02] relative ${mode === 'dark' ? 'bg-gray-800/80 backdrop-blur-sm border border-gray-700' : 'bg-white shadow-xl'
                  }`}
              >
                ...
                <div className={`h-40 bg-gradient-to-r ${index % 2 === 0
            ...
            mode === 'dark' ? 'text-gray-200' : 'text-gray-700'
                  ...
                  <div className={`text-sm font-medium mb-2 ${mode === 'dark' ? 'text-gray-400' : 'text-gray-500'
                    }`}>
                    {event.date}
                  </div>
                  <h3 className={`text-xl font-bold mb-2 ${mode === 'dark' ? 'text-white' : 'text-gray-900'
                    }`}>{event.title}</h3>
                  <p className="mb-4">{event.description}</p>
                  <Link
                    to="/events"
                    className={`inline-flex items-center font-medium ${index % 2 === 0
          ...
          <div className="text-center mt-12">
            <Link
              to="/events"
              className={`inline-flex items-center px-6 py-3 rounded-lg font-medium transition-all ${mode === 'dark'
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            {featuredPosts.map((post, index) => (
              <div
                key={index}
                className={`p-6 rounded-lg ${mode === 'dark' ? 'bg-gray-700' : 'bg-white shadow-md'
              ...
              >
                ...
                <Link
                  to="/blog"
                  className={`inline-flex items-center ${mode === 'dark' ? 'text-yellow-400' : 'text-orange-600'
                    } hover:underline`}
                >
          ...
          <div className="text-center mt-12">
            <Link
              to="/blog"
              className={`inline-flex items-center px-6 py-3 rounded-lg ${mode === 'dark'
